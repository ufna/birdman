...
* **QoS-эхо — наоборот, общее.** Респондер безадресный (байт-зеркало ≤64б), rtt
  ...
  таргет на пару (регион, ip). Держит его ровно один инстанс, остальные пишут
  `qos_echo_addr: off`. Цена: пока владелец лежит, ping-таргет бокса тёмный для
  всех его нод (наблюдаемо — на владельце горит `NodeDown`).

Роль падает **до записи на диск**, если ноды бокса делят имя, порт, каталог,
токен или namespace, или если владельцев эха не ровно один. Проверяется
...
⚠️ Граница гарда, названная честно: флаг `birdman_devdeploy_enabled` доезжает
до роли агента только там, где в плее есть роль деплоера, то есть в
`dev-node.yml`. В `add-node.yml` её нет, флаг не определён, и для УДАЛЁННОЙ
ноды гард выключен — хотя деплоер её агента катает (цели
`GET /v1/nodes?env=dev` фильтруются по окружению, а не по боксу). Пока это не
закрыто (#109 (1070)), перед прогоном `add-node.yml` по ноде стенда либо
выключай деплоер, либо задай ноде `birdman_devdeploy_enabled: true` в
инвентаре.
