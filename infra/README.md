...
`loop_var`, внутрь `block:` включительно) и требует: путь либо мирочитаем, либо
принадлежит uid'у образа. Uid **добывается у самого образа** (`docker run
--entrypoint id`), а не берётся из таблицы: таблица разъехалась бы с образом при
первом же апгрейде — а апгрейд образа и есть второй способ наступить на те же
грабли.

Тома читаются в **обеих** легальных формах записи — короткой строкой
(`/etc/x:/etc/x:ro`) и длинной (`type: bind` / `source:` / `target:`). Это не
косметика: первая редакция сторожа брала только короткую, и конфиг `0600
root:root`, смонтированный длинным синтаксисом, проезжал мимо **молча и
зелёно**, а compose мастера при этом рапортовал «bind-маунтов нет вовсе». Запись
тома, которую классифицировать нельзя, теперь роняет прогон, а не выбрасывается.

Форму определяет то же, что определяет её у compose: у короткой записи —
**вид источника** (со слэшем путь, без слэша имя тома), у длинной — **`type`**,
который docker и сам требует. Вторая редакция сторожа мерила длинную запись по
виду имени и снова теряла молча — `type: bind` + `source: pg-tuning.conf`
compose принимает и разворачивает в настоящий bind
`<каталог-проекта>/pg-tuning.conf`, а сторож записывал такую строку в
именованные тома и отвечал «маунтов нет». Список форм больше не держится на
памяти ревьюера: каждая пиновка — кейс в `./infra/ci/tests/run.sh` (раздел
«формы записи тома»), у каждой формы ровно один законный исход — маунт или
громкий отказ.

Известная **не** закрытая дыра того же класса: хостовый путь, привязанный не в
`services.*.volumes`, а верхнеуровневым томом с
`driver_opts: {type: none, device: /srv/…, o: bind}` — сторож его пока не видит
(tracker #1096).
...
Честные границы (все — в сторону строгости): групповые биты не учитываются
(`group_add` контейнера отсюда не виден); контейнер от root, который по
...
на которое стоит опираться; **относительная** хостовая сторона маунта
(`./conf:/etc/conf`) отвергается, потому что считается от каталога
compose-проекта на боксе, а угадывать этот каталог значило бы завести вторую
правду о раскладке. Раскладка, попавшая в границу, будет отвергнута: сначала
правится сторож, молча она не проедет.
