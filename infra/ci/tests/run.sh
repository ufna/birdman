...
#     (tracker #1120): разбирает её не наш список, а модель, списанная с
#     ЗАМЕРЕННОГО поведения docker (исходников docker отсюда никто не читал).
#     Кейсы двери 8 — ниже, сразу за кейсами `logging`: критерий у неё общий с
...
#     расходился бы он с docker молча. #1122 довела замер до конца и вывод
#     подтвердила: до демона доезжает ПУТЬ, а не содержимое (зеркально двери
#     8), путеносных опций шесть на трёх драйверах, а драйвер может быть
#     ПЛАГИНОМ со своими именами — то есть набор неполон по построению.
#     Четыре кейса ниже, чтобы решение было видно исполняемым фактом, а не
#     только словами в шапке.
echo
...
# Кейсы пришпиливают именно то, что НЕ закрыто, и это сделано намеренно: иначе
...
# на «похоже на путь» покраснел бы на легальном. Читает эти пути ДЕМОН от root,
# а не контейнер: класса #1072 (контейнер не прочитал свой конфиг) тут не
# возникает.
# ЕСЛИ ЭТИ КЕЙСЫ КОГДА-НИБУДЬ ПОКРАСНЕЮТ — значит кто-то научил сторожа
# заходить внутрь значений `logging`; тогда правится не кейс, а шапка.
#
# Кейсов ЧЕТЫРЕ, и это не запас прочности, а протокол замера #1122 на живом
# демоне (27.5.1, `linux/arm64`, Docker Desktop, 21 прогон `up -d`): каждый
# пришпиливает ОДНО измеренное свойство, из-за которого двери 9 нет. Развилка
# «дверь или решение» там пройдена с числами — переоткрывать её с нуля не надо,
# разбор целиком лежит в шапке mounted_config_access.py.
...
YML
# Три опции, которых #1122 звала НЕПОКРЫТЫМИ, — покрыты: демон читает эти файлы
# на ЗАПУСКЕ задачи («could not read CA certificate», «could not load X509 key
# pair»), причём РАНЬШЕ tcp-дозвона (контроль без tls-опций даёт «dial tcp …
# connection refused»). То есть у ОДНОГО syslog путеносных опций ЧЕТЫРЕ.
vol_case "logging: syslog-tls-* пути — зелено ОСОЗНАННО" 0 "проверено маунтов: 1" <<-'YML'
	services:
	  db:
	    image: postgres:16
	    volumes:
	    ...
	    logging:
	      ...
	      options:
	        ...
	        syslog-tls-ca-cert: "/srv/fixture/secret.conf"
	        syslog-tls-cert: "/srv/fixture/secret.conf"
	        syslog-tls-key: "/srv/fixture/secret.conf"
YML
# Имена путеносных опций РАЗНЫЕ у разных драйверов, и держит их ДЕМОН, а не
# схема: `fluentd-address` и `splunk-capath` замерены (оба — отказ на запуске
# задачи). Драйвер вдобавок может быть ПЛАГИНОМ со своими именами опций
# (замерено: «error looking up logging plugin … not found»), так что список
# имён неполон ПО ПОСТРОЕНИЮ — держать его пришлось бы нам.
vol_case "logging: пути у ДРУГИХ драйверов — зелено ОСОЗНАННО" 0 "проверено маунтов: 2" <<-'YML'
	services:
	  db:
	    image: postgres:16
	    volumes:
	    ...
	    logging:
	      driver: fluentd
	      options:
	        fluentd-address: "unix:///srv/fixture/secret.conf"
	  web:
	    image: nginx:1.27
	    volumes:
	      - /srv/fixture/pg.conf:/etc/pg.conf:ro
	    logging:
	      driver: splunk
	      options:
	        splunk-url: "https://logs.example:8088"
	        splunk-capath: "/srv/fixture/secret.conf"
YML
# А ЭТОТ КЕЙС — про то, почему нельзя обойтись БЕЗ списка имён. Значение
# выглядит абсолютным путём и путём НЕ ЯВЛЯЕТСЯ: замерено, `json-file` +
# `tag: "/prod/app/{{.Name}}"` даёт RC=0 и бегущий контейнер. Тот же класс —
# `awslogs-group: /ecs/…`. Скан «похоже на путь → красное» покраснел бы здесь,
# то есть на легальном, а ложный отказ хуже дыры, цена которой сегодня ноль
# (`logging` не встречается ни в одном из пяти compose-шаблонов ролей).
# Прежний пример шапки (`syslog-address: unix:///dev/log`) слабее: его снял бы
# пропуск под `/dev/`, как у дверей 5 и 7. Этот не снимает ничто.
vol_case "logging: значение похоже на путь, но путём не является" 0 "проверено маунтов: 1" <<-'YML'
	services:
	  db:
	    image: postgres:16
	    volumes:
...
YML
# ФОРМА С `=`: docker режет строку сначала по `=` и только потом по `:` — это
# МОДЕЛЬ, выведенная из ЗАМЕРОВ, а не из исходников docker (их никто не читал).
# Замерено: `seccomp=/nope/absent.json` даёт то же
