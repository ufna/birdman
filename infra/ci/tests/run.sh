...
# ─── ГЕЙТ 7: ЗНАЧЕНИЯ инертных ключей сервиса (tracker #1119) ───────────────
...
# замер эти три РАЗДЕЛИЛ, а не уравнял:
#
...
#   · `logging.options` и `security_opt` схема НЕ закрывает: `options:
#     {выдумка: /путь}` и `security_opt: ["выдумка:/путь"]` compose принимает
#     (RC=0). Написать белый список и тут МОЖНО — нельзя сослаться на схему, то
#     есть держать его пришлось бы нам, и расходился бы он с docker молча.
#     Поэтому они оставлены СОЗНАТЕЛЬНО — с двумя кейсами ниже, чтобы решение
#     было видно исполняемым фактом, а не только словами в шапке.
echo
echo "── значения инертных ключей: blkio закрыт, logging и security_opt — решение"
...
# ── ДВА ОСТАВШИХСЯ: зелёное здесь — РЕШЕНИЕ, а не недосмотр ─────────────────
# Кейсы пришпиливают именно то, что НЕ закрыто, и это сделано намеренно: иначе
# следующий проход по шапке снова прочтёт «остаток» и заведёт десятую карточку,
# а прочитав — не сможет отличить решение от забывчивости. Закрывать их
# отказом нельзя: набор значений схемой не закрыт (замерено), а скан значений
# на «похоже на путь» покраснел бы на совершенно легальном —
# `syslog-address: unix:///dev/log` и seccomp-профиль, который кладёт ОС.
# Читателем у обоих выступает демон/ядро от root, а root читает что угодно:
# класса #1072 (контейнер не прочитал свой конфиг) тут не возникает.
# ЕСЛИ ЭТИ ДВА КЕЙСА КОГДА-НИБУДЬ ПОКРАСНЕЮТ — значит кто-то научил сторожа
# заходить внутрь этих значений; тогда правится не кейс, а шапка.
...
YML
vol_case "security_opt: seccomp-путь — зелено ОСОЗНАННО" 0 "проверено маунтов: 1" <<-'YML'
	services:
	  db:
	    image: postgres:16
	    volumes:
...
if [ "$rc" = 0 ] && printf '%s' "$out" | grep -qF "7+5+76=88"; then
	echo "ok   ключи сервиса: 7 дверей + 5 отвергаемых + 76 инертных = 88, пересечений нет"
	pass=$((pass + 1))
else
