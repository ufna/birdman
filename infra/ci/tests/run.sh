...
#
# НАСТОЯЩАЯ ПРИЧИНА, ЗАМЕРЕНА В ЛОБ: bash 3.2 (/bin/bash на macOS) НЕ ВЫСТАВЛЯЕТ
# `$?` перед запуском EXIT-trap'а при выходе по НЕОПРЕДЕЛЁННОЙ ПЕРЕМЕННОЙ —
# `trap 'echo "$?"' EXIT; : "${nope}"` печатает 0. Поэтому НИ ОДИН trap,
# читающий `$?`, этот режим не восстанавливает: `st=$?` захватывает ноль и
# честно отдаёт его наружу. (Первая попытка чинить именно так — `st=$?; rm;
# exit "$st"` — была поведенчески НЕОТЛИЧИМА от голого `rm -rf`, замерено по
# всем восьми режимам; и диагноз «rm затирает код» был неверен: при голом
# `rm -rf` `set -e`, `exit N`, pipefail и честный красный финал все отдают
# ненулевой код.) Режим версионно-зависим: на bash 5, то есть на CI, он, по
# замеру на ревью #1089, отдаёт 1 штатно — ЗДЕСЬ ЭТО НЕ ПЕРЕМЕРЕНО, bash 5 на
# машине нет. То есть дыра локальная, мерж-гейт ею не врал; но ловится она там
# же, где раннеры гоняют руками.
#
...
#
# Кейса ДВА, потому что у часового ровно два способа соврать: не покраснеть,
# когда должен, и придавить чужой ненулевой код своей единицей.
if [ -z "${BIRDMAN_TRAP_SELFTEST:-}" ]; then
	...
	trap_case() {
		...
		mkdir -p "$dir"
		awk -v ins="$inject" '{print} /^trap .*EXIT$/ {print ins}' "$self" >"$dir/run.sh"
		chmod +x "$dir/run.sh"
		# Молчаливо не вписавшаяся смерть означала бы кейс, проверяющий не то,
		# — то есть ровно тот отказ, ради которого этот блок и написан.
		if ! grep -qF "$inject" "$dir/run.sh"; then
			echo "FAIL раннер: $name — смерть не вписалась, строка trap'а сменила вид"
			fail=$((fail + 1))
			return
		fi
		local rc=0
