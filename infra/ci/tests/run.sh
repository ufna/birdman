...
# (замерено сигналом по живому прогону: TERM дважды и INT — мутантов в дереве
# не осталось ни разу; SIGKILL трапа не запускает по определению, на него
# страховка в .gitignore).
#
...
# agent_dev RC=125 (master_dev докера не просит вовсе и даёт ALL OK). Ни один
# кейс до этих шагов не доходит: два убивают мутанта строкой сразу за trap'ом,
# третий вырезает тело целиком, а в голове раннера до trap'а докерных шагов нет
# — только резолв путей, чтение defaults роли и `mktemp -d`. Замер сходится:
# весь блок зелёный именно при мёртвом демоне.
if [ -z "${BIRDMAN_TRAP_SELFTEST:-}" ]; then
	echo
	...
	if [ "$pinned" -eq 0 ]; then
		echo "FAIL часовой раннеров ролей: под критерий (-e + -u + EXIT-trap) не попал ни один раннер — раскладка ролей уехала или часовой снесён везде разом"
		fail=$((fail + 1))
	else
		echo "     (раннеров под часовым: $pinned)"
	fi
fi
