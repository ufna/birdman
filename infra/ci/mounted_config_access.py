...
попадает в контейнер. Двери 1-4 в ролях не встречались; ровно так же не
встречался и дефект #1072 в трёх composes из четырёх — за сутки до того, как
встретился. А двери 5 и 6 в ролях УЖЕ ЖИЛИ, и обе шли мимо сторожа зелёными:
  ...
  6. `env_file:` / `label_file:` — разбираются (`host_tool_files()`), но ДРУГИМ
     ...
     неприменим (живой мониторинг: `grafana.env` лежит 0600 root:root и
     РАБОТАЕТ; применить #1072 значило бы покраснеть на работающей раскладке —
     замерено, наивный вариант так и делал), а осмысленный вопрос ровно один:
     кладёт ли путь таска роли. Длинная форма с `required: false` — пропуск:
     такой файл compose не требует (замерено там же).
...
РАССУЖДЕНИЕ. Прошлая редакция этой шапки называла уровень неподъёмным («у
сервиса десятки законных ключей, отказ на неизвестном краснел бы на живых
composes»). Оба звена промерены и оба неверны:

  · набор КОНЕЧЕН и закрыт схемой самого compose: `docker compose config`
  ...
  · цена НУЛЕВАЯ: восемь настоящих рендеров репозитория (мастер; агент на боксе
    из одной и из двух нод — по два compose каждый; мониторинг; оверлей спок и
    хаб) используют 13 ключей сервиса, и все 13 понятые. Вывод сторожа на этих
    рендерах до и после перехода отличается ТОЛЬКО дописанными строками про
    ранее невидимые записи (`/dev/net/tun` у оверлея, `grafana.env` у
    мониторинга); ни один код возврата не изменился.
...
ГДЕ РОСТ НЕ ГРОМКИЙ, И ЭТО ЗАМЕРЕНО, А НЕ ЗАБЫТО. Остаток ровно один и он
НАЗВАН: `logging` оставлен ИНЕРТНЫМ, хотя одна его форма хостовый путь несёт —
`options: {syslog-address: "unix:///dev/log"}`. Класс #1072 к ней не относится:
сокет открывает демон docker от root, а не контейнер. Отвергать же ключ целиком
дороже пользы — покраснел бы совершенно нормальный `json-file` + `max-size`,
которого в ролях сегодня нет, но который появится первым. Так что про этот
остаток сказано, как есть, и не обещано большего.
