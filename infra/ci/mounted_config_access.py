...
ТРИ ДВЕРИ РАЗБИРАЮТСЯ, ЧЕТВЁРТАЯ ОТВЕРГАЕТСЯ (tracker #1097). Перебор форм в
`services.*.volumes` был закрыт ровно настолько, насколько его кто-то вспомнил,
— и всё это время мимо сторожа шли ещё двери, которыми хостовый путь попадает в
контейнер. Ни одна из них сегодня в ролях не встречается; ровно так же не
...
РОСТ ФОРМ ГРОМКИЙ НА ДВУХ УРОВНЯХ — И ТОЛЬКО НА НИХ. Перебирать формы бесконечно
...
построению» — но ровно на этих двух уровнях, и не шире.

ГДЕ РОСТ НЕ ГРОМКИЙ, И ЭТО ЗАМЕРЕНО, А НЕ ЗАБЫТО: список ключей САМОГО СЕРВИСА
не закрыт. Сторож знает пять — `volumes`, `configs`, `secrets` разбирает,
`volumes_from` и `extends` отвергает, — а ключ вне этой пятёрки до него не
доезжает вовсе. Замерено на `devices: [/srv/…:/dev/x]`: RC=0, запись не попала
ни в маунты, ни в отказ, сторож её не увидел. Дверью класса #1072 этот ключ
сегодня не является (в `devices` пишут устройства, а их роль тасками не кладёт)
— и в этом же вся цена закрытия уровня: у сервиса десятки законных ключей
(image, environment, ports, healthcheck, deploy, logging, sysctls…), поэтому
«неизвестный ключ сервиса = отказ» краснел бы на живых composes ролей, а не на
новых дверях, и требует своей карточки с честным перебором, а не строчки здесь.
Так что про этот уровень сказано, как есть, и не обещано большего.
