...
РОСТ ФОРМ ГРОМКИЙ НА ЧЕТЫРЁХ УРОВНЯХ — И ТОЛЬКО НА НИХ. Перебирать формы
бесконечно нельзя, но можно сделать НЕперебранную форму красной. Перечислено то,
что сторож понимает, а всё остальное роняет прогон:
  ...
  · ключи ВНУТРИ ОДНОЙ ЗАПИСИ длинной формы (`entry_keys()`, tracker #1115) —
    уровень НИЖЕ ключей сервиса и самый глубокий. Сам ключ сервиса сторож уже
    ...
    выдумка: 1}` давало RC=0 и «проверено маунтов: 1». Записей ПЯТЬ, и наборы
...
построению» — но ровно на этих четырёх уровнях, и не шире.
...
ГДЕ РОСТ НЕ ГРОМКИЙ, И ЭТО ЗАМЕРЕНО, А НЕ ЗАБЫТО. Остатков ТРИ, и они названы
поимённо. Их было ЧЕТЫРЕ: первая редакция этого абзаца объявляла остаток ОДНИМ,
два независимых прохода по ней нашли остальные три, а четвёртый — ключи внутри
записи длинной формы — закрыт белым списком выше (#1115). ОСТАВШИЕСЯ ТРИ ЖИВЫ, и
механизм #1115 их НЕ закрывает, даже попутно: он разбирает ЗАПИСИ длинных форм, а
эти три — ЗНАЧЕНИЯ инертных ключей сервиса, внутрь которых сторож не заходит
вовсе. Своих карточек у них пока нет. Все три — инертные ключи, у которых
хостовый путь есть, а класса #1072 нет, потому что читает его не контейнер:
...
появится первым. Так что про эти остатки сказано, как есть, и не обещано
большего: четыре уровня выше закрыты по построению, а ЭТИ ТРИ — нет.
