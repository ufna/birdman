...
СЕМЬ ДВЕРЕЙ: ШЕСТЬ РАЗБИРАЮТСЯ, ЧЕТВЁРТАЯ ОТВЕРГАЕТСЯ (#1097, #1107, #1119).
Перебор форм в `services.*.volumes` был закрыт ровно настолько, насколько его
кто-то вспомнил, — и всё это время мимо сторожа шли ещё двери, которыми в
compose въезжает ХОСТОВЫЙ ПУТЬ. Именно так, а не «путь попадает в контейнер»:
это было бы неправдой про три двери из семи. У двери 6 файл читает сам compose
на хосте, у двери 7 путь вообще никуда не монтируется (это узел для ядра), а у
двери 5 хостовая сторона под `/dev/` — сознательный пропуск. Общее у всех семи
одно: хостовый путь в документе есть, и сторож обязан его УВИДЕТЬ, — а уже
какой критерий к нему приложить, решается у каждой двери отдельно и сказано у
...
Перечислено то, что сторож понимает, а всё остальное роняет прогон:
  ...
  · ключи САМОГО СЕРВИСА (`service_keys()`, tracker #1107): кроме разобранных
    ...
    `label_file`), отвергаемых (`volumes_from`, `extends`, `build`, `develop`,
    `credential_spec`), 76 поимённо перечисленных инертных и расширений `x-`,
  ...
  · ЗНАЧЕНИЯ ключей сервиса, которые числились ИНЕРТНЫМИ (tracker #1119) —
    пятый уровень, и он закрыт НЕ ЦЕЛИКОМ, а на одном ключе из трёх (закрытый
    после этого из инертных и вышел — он теперь дверь 7). Это не полумера от
    усталости:
    ...
    тот же вопрос задан заново — и ответ оказался РАЗНЫМ у разных ключей:
      – `blkio_config` схема закрывает ЦЕЛИКОМ, и собственные ключи, и ключи
        ...
        дверь 7;
      – `logging.options` и `security_opt` схема НЕ закрывает, и они оставлены
        СОЗНАТЕЛЬНО. Разбор — ниже, в разделе «где рост НЕ громкий».

Это и есть разница между «список закрыт, пока кто-то помнит» и «список закрыт по
построению» — но ровно на четырёх уровнях целиком плюс один ключ на пятом, и не
...
ГДЕ РОСТ НЕ ГРОМКИЙ, И ЭТО ЗАМЕРЕНО, А НЕ ЗАБЫТО. Остатка ДВА, и они названы
поимённо. Их было ЧЕТЫРЕ, потом ТРИ: первая редакция этого абзаца объявляла
остаток ОДНИМ, два независимых прохода по ней нашли остальные три, четвёртый —
ключи внутри записи длинной формы — закрыт белым списком выше (#1115), а
`blkio_config` закрыт дверью 7 (#1119). ОСТАВШИЕСЯ ДВА ЖИВЫ, и это ТЕПЕРЬ
РЕШЕНИЕ, А НЕ ОСТАТОК: под ними лежит замер, а не ненаписанный код.

  · `logging`: `options: {syslog-address: "unix:///dev/log"}` — сокет открывает
    демон docker от root;
  · `security_opt`: `["seccomp:/путь/profile.json"]` — профиль читает демон и
    отдаёт ядру его СОДЕРЖИМОЕ.

Почему они НЕ закрываются — три причины, и они РАЗНОГО ВЕСА. Первая замер;
вторая — расчёт, опирающийся на замер соседних дверей; третья — то, КАК МЫ
ЧИТАЕМ DOCKER, и замера под ней нет вовсе (см. «честная граница» ниже). Сказано
так намеренно: третья причина выглядит самой убедительной и проверить её отсюда
нельзя, поэтому выдавать её за измеренную было бы ровно тем, против чего этот
файл написан.

  · БЕЛЫЙ СПИСОК ЗДЕСЬ БЫЛ БЫ НАШЕЙ СОБСТВЕННОЙ ПРАВДОЙ, а не схемой — и это
    главное отличие от четырёх уровней выше. Там набор был конечен, потому что
    его закрывала схема compose; здесь она его НЕ закрывает. Замерено на v2.32.4
    client-side: `logging: {driver: syslog, options: {выдумка: /srv/…}}` — RC=0,
    и `security_opt: ["выдумка:/srv/…"]` — тоже RC=0 (как и голый `["/srv/…"]`).
    То есть имена опций логгера и строки security_opt — ОТКРЫТЫЕ множества.
    Закрыты у `logging` только его собственные ключи (`выдумка` рядом с
    `driver`/`options` — RC=15), но хостовый путь ездит не в них. Написать
    список всё равно МОЖНО — нельзя только сослаться на схему, а значит его
    пришлось бы держать нам, и он расходился бы с docker молча. Будь ответ
    обратным, эти два закрылись бы вместе с `blkio_config` и тем же кодом;
  · СКАН ЗНАЧЕНИЙ («любая строка, похожая на абсолютный путь, — красная») даёт
    ЛОЖНЫЙ ОТКАЗ НА ЛЕГАЛЬНОМ. Покраснели бы `syslog-address: unix:///dev/log`
    — самая обыкновенная syslog-раскладка — и `seccomp:` на профиле, который
    кладёт ОС, а не роль. Цена такой ошибки в этой линии уже измерена и записана
    абзацем выше: наивный критерий дверей 5 и 6 давал ТРИ ложных отказа на живых
    рендерах (мониторинг и оба режима оверлея). Число унаследовано оттуда, а не
    перемерено здесь;
  · А КРИТЕРИЯ #1072 ТУТ, ПОХОЖЕ, НЕТ ВООБЩЕ — и это довод, который был бы
    сильнее двух первых, если бы его было чем подтвердить ОТСЮДА. Нечем:
    держится он на том, что docker делает с этими ключами, а демона в прогоне
    нет. Читается это так: в контейнер не монтируется ни то, ни другое — опции
    лог-драйвера открывает демон (контейнер пишет в stdout и этих путей не
    открывает вовсе), а seccomp-профиль демон читает и отдаёт ядру содержимым.
    Раз в контейнер ничего не заезжает, то и вопроса «прочтёт ли его контейнер»
    не возникает. ДОБАВОЧНАЯ посылка «а читает демон от root, значит права ни
    при чём» здесь СЛАБЕЕ и намеренно не несёт нагрузки: root — свойство НАШЕЙ
    раскладки (дверь 6 в этой же шапке оговаривает его честно — «у нас root»),
    и при rootless-демоне она неверна. Вывод стоит и без неё. Дверь 6 —
    прецедент ровно этой формы: там критерий #1072 тоже неприменим, и ей дали
    ДРУГОЙ; здесь другого нет.

Отвергать эти ключи целиком по-прежнему дороже пользы: покраснел бы совершенно
нормальный `logging: json-file` + `max-size`, которого в ролях сегодня нет, но
который появится первым. А завести СВОЙ белый список лог-драйверов и их опций
значило бы держать ВТОРУЮ ПРАВДУ про docker — ту, которую не закрывает никакая
схема и которую поддерживаем мы. Решения этой формы шапка отвергает уже дважды —
про роле-локальный рендер в самом начале («один generic-рендерер был бы второй
правдой о раскладке») и про `docker compose config` сразу ниже, — и здесь ответ
тот же.

ЧЕСТНАЯ ГРАНИЦА ЭТОГО РЕШЕНИЯ. Оно опирается на то, что делает docker с этими
двумя ключами, — а это НЕ проверяется отсюда и проверено быть не может: демона
в прогоне нет. Поправка к предыдущей редакции, которая тут утверждала лишнее:
`docker compose config` seccomp-профиль НЕ ЧИТАЕТ. Замерено — несуществующий
путь проезжает молча (RC=0), а существующий НЕ подставляется содержимым, путь
уезжает в вывод как есть; контроль тем же методом: `env_file` на несуществующий
файл валится громко («env file … not found»). Значит критерий двери 6 («файл
обязан существовать, замерено client-side») к seccomp приложить НЕЛЬЗЯ — под
ним не было бы замера. Что остаётся правдой без демона: у обоих ключей путь
есть, сторож в них не заходит, и оба кейса стоят в infra/ci/tests/run.sh
ЗЕЛЁНЫМИ — намеренно, чтобы решение было видно исполняемым фактом. Покраснели —
скорее всего кто-то научил сторожа заходить внутрь этих значений (хотя уронить
их может и правка `service_keys()` или фикстуры); если причина именно та,
правится не кейс, а этот абзац.
...
# без остатка: 7 разбираются дверями, 5 отвергаются выше, 76 лежат здесь.
...
# `blkio_config` из этого списка УЕХАЛ в разбираемые (tracker #1119): он нёс
# хостовые пути в `path`, и лежать среди «не несут хостового пути ВОВСЕ» ему
# было не по чину.
