...
#   5. #1089 (хвост #1072) — конфиг, который роль монтирует в контейнер, обязан
#      этим контейнером ЧИТАТЬСЯ. Общий сторож (infra/ci/), рендер отсюда: у
#      роли ДВА compose-проекта (vector и нодовый vmagent), и пути маунтов у
#      разных нод бокса разные.
set -euo pipefail
...
# Роль мониторинга положила alertmanager.yml 0600 root:root в контейнер,
# бегущий от nobody: краш-луп с рождения, 2380 рестартов, роль зелёная. Здесь
# тот же критерий по composes ЭТОЙ роли — сегодня они чистые, и задача сторожа
# в том, чтобы завтрашний маунт (или апгрейд образа на non-root) не приехал
# молча. Оба бокса, потому что пути маунтов у нод РАЗНЫЕ: у второй ноды свой
# log_dir, и накрыть надо каждый.
#
# vars.json — факты, выведенные tasks/instances.yml этим же прогоном: часть
# путей роль кладёт внутри пер-инстансного include, и по defaults их не найти.
echo "── mounted config access"
for box in one two; do
  "$repo/infra/ci/mounted-config-access.sh" --role "$role" --vars "$work/$box/vars.json" \
    "$work/$box/compose.yml" "$work/$box/vmagent-compose.yml"
done
