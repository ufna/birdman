...
// платформенные ПО РОЖДЕНИЮ события.
//
...
// insertEvent, поэтому платформенной строкой в ленту привязанного ключа
// законно приезжают события, которым атрибуции не досталось ПО СУЩЕСТВУ
// (бекапы, CA, реестры — у них проекта нет).
//
// ИСТОРИИ УДАЛЁННОГО ПРОЕКТА СРЕДИ НИХ БОЛЬШЕ НЕТ (tracker #1083), хотя раньше
// она приезжала сюда целиком: FK объявлен `on delete set null`, поэтому после
// удаления проекта его строки становились null-проектными ПОСТФАКТУМ и
// проезжали не скрывающее сужение легально. Теперь «платформенное» значит «ни
// `project_id`, ни снимка слага» (миграция 000020), и осиротевшую строку
// отсекает та самая выборка, которую двигает этот курсор — `ListEventsAfter`,
// тем же условием, что и лента: правила у двух входов в одну таблицу обязаны
// совпадать, иначе сужение ленты обходится подпиской на стрим (tracker #999).
//
// Событий жизненного цикла ключа среди приезжающих тоже больше нет:
// `apikey_revoked`/`apikey_purged` теряли привязку не по существу, а потому что
// выборка стора её не читала, — починено в tracker #1017. Остаток —
