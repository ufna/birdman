...
// входом стал гейт, а исключение было громким. С #995 прямых вызывающих у неё
// НЕ ОСТАЛОСЬ ВОВСЕ: последние два (`GET /v1/alerts/active` и `/history`)
// переведены на `tenantScope`, а не-скрывающий контракт алертов («платформенный
// алерт виден под любым фильтром») уехал туда, где ему и место, — в ФИЛЬТР
// (`httpapi.alertVisibility`), а не в разбор параметра. Единственный вызывающий
// — `tenantScope` выше; появился второй — это исключение, и оно требует причины
// в этом комментарии.
//
