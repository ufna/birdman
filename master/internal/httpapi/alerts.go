...
// втором случае наружу уходит вся полезная нагрузка проектного алерта, а не
// адрес узла. Сегодня такое правило ровно одно — TickDegraded: проект ему
// доклеивается join'ом с birdman_server_info, а ветка `unless` намеренно
// оставляет алерт гореть без лейбла, когда info-серии нет (master лёг, БД
// моргнула), и тогда привязанный ключ видит server_id чужого дедика и его
// игровой тик. Замерено на живом VM v1.102.1: после #1008 так деградируют
