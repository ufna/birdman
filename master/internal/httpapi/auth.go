...
func newAuthenticator(st *store.Store) *authenticator {
	return &authenticator{st: st, cache: map[[32]byte]cachedKey{}}
}

// authenticate resolves the bearer token to an API key. bcrypt verification
// results are cached (sha256(token) → scopes) so hot paths like /v1/allocate
// stay well under the 50ms SLO.
func (a *authenticator) authenticate(r *http.Request) (store.APIKey, bool) {
...
// admin, which implies everything).
