...
func (s *Store) ListVersions(ctx context.Context) ([]Version, error) {
	rows, err := s.Pool.Query(ctx, `
		select v.id::text, v.project_id::text, p.slug, v.semver, v.image_ref, v.channel, v.state, v.created_at
	...
	for rows.Next() {
		var v Version
		if err := rows.Scan(&v.ID, &v.ProjectID, &v.Project, &v.Semver, &v.ImageRef, &v.Channel, &v.State, &v.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, v)
	}
