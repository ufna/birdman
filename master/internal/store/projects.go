...
func (s *Store) DeleteProject(ctx context.Context, slug, confirm string) (DeleteProjectResult, error) {
	...
	// Роллапы адресуются СЛАГОМ проекта: project_id (uuid) в таблице нет, но
	// колонка `project` есть и входит в первичный ключ (миграция 000017,
	// (day, region, semver, env, project)). Адресация по одному лишь имени
	// окружения ошибалась бы в ОБЕ стороны, и обе — молча: dev/prod сидируются
	// каждому проекту, поэтому «имя занято другим» истинно почти всегда и не
	// удалялось бы ничего (новый проект с тем же слагом унаследовал бы чужую
	// статистику), а на уникальном имени, наоборот, снесло бы строки чужих
	// проектов.
	if _, err := tx.Exec(ctx,
		`delete from match_stats_daily where project = $1`, slug); err != nil {
		return res, err
	}

	// Пик CCU лежит в СОСЕДНЕЙ таблице, и та же миграция 000017 дала ей ту же
	// колонку `project` (PK стал (day, project)). Читает её проектным срезом
	// store.RollupPeakCCU — им закрыта иммутабельная часть окна
	// GET /v1/stats/overview|cost. Без этой уборки новый проект, созданный с
	// ТЕМ ЖЕ слагом (переименования слага нет by design, пересоздание — штатный
	// путь), унаследовал бы пик мёртвого: ровно та же порча, что была у
	// match_stats_daily выше, только в соседней таблице.
	//
	// СТРОКУ project = '' ТРОГАТЬ НЕЛЬЗЯ, и это не то же самое, что ''-строка в
	// match_stats_daily. Здесь она не просто «не атрибутировано», а
	// платформенный пик за день И маркер «день посчитан»: на наличии ключа в
	// выдаче RollupPeakCCU держится различение «день был пустой» и «день ещё не
	// роллапился». Условие по слагу гарантирует это само (пустой слаг не
	// проходит валидацию имени проекта) — но следующему читателю, которому
	// захочется обобщить запрос до «всех строк дня», это надо знать заранее.
	//
	// Каскаду ОКРУЖЕНИЯ такой уборки не нужно: у пика CCU нет измерения env —
	// решение I5, прямо записанное в 000017 (проекты — непересекающиеся
	// тенанты, а окружения делят одну ёмкость флота).
	if _, err := tx.Exec(ctx,
		`delete from match_ccu_daily where project = $1`, slug); err != nil {
		return res, err
	}
