...
func (c *dbCollector) Collect(ch chan<- prometheus.Metric) {
	...
	// (бекапы, CA, сессии) — оно принадлежит не проекту, а установке. Причём
	// платформенное ПО РОЖДЕНИЮ: после #1083 «пусто» здесь больше не значит
	// «project_id is null», потому что осиротевшие строки в выборку не попадают
	// вовсе — см. следующий абзац.
	//
	// ИСТОРИЯ УДАЛЁННОГО ПРОЕКТА В ЭТУ ВЫБОРКУ НЕ ВХОДИТ (tracker #1084).
	// Серия деривится ЗАПРОСОМ на каждом скрейпе, а не копится в процессе,
	// поэтому `on delete set null` переносил ВЕСЬ исторический объём проекта из
	// {kind, project="A"} в {kind, project=""} за ОДИН скрейп: платформенный
	// Counter ступенчато вырастал на историческое число, и правило
	// `increase(birdman_events_total{kind="crash_loop"}[5m]) > 0` читало это как
	// всплеск — critical с пустым $labels.project на рутинном удалении проекта.
	// Гейт свежести скрейпа (#1063) тут не спасает и не может: скрейп исправен,
	// растёт само значение. Правило отличить перенос от настоящего всплеска не
	// в состоянии — обе картинки суть рост значения непрерывно скрейпимой
	// серии, — поэтому чинится источник.
	//
	// Условие читается как «строка живого проекта ИЛИ платформенная ПО
	// РОЖДЕНИЮ»; отсекается ровно одно состояние — осиротевшая строка
	// (project_id обнулён каскадом, снимок слага остался, миграция 000020).
	// Осиротевшие события не пропадают из аудита — они пропадают с МЕТРИЧЕСКОЙ
	// поверхности, и это то же направление, что у #1066: метрики описывают
	// живое состояние, а история удалённого проекта неадресуема ни одним
	// правилом и ни одному дежурному не действие. Отдельная серия под мёртвых
	// сохранила бы числа ценой монотонно растущей кардинальности.
	//
	// Серия {kind, project="A"} при этом просто исчезает (нулевую базу для неё
	// тоже больше никто не заводит — слаг ушёл из projects), и алерта это не
	// поднимает: increase() по константным сэмплам даёт 0, а пересоздание
	// проекта с тем же слагом приезжает нулём, то есть counter reset'ом.
	if erows, err := c.st.Pool.Query(ctx, `
		...
		where e.project_id is not null or e.project_slug is null
