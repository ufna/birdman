...
// TestBackupMetrics: the six Backups v1 series are derived from Postgres on
...
// backup_settings seed, last_success=0 (never). After one ok run (S3 uploaded)
// and one error run: the last-success/size/s3 gauges are set and runs_total
// splits ok=1/error=1. Reuses the package test harness (TestMain, findGauge,
// findCounter in metrics_test.go).
func TestBackupMetrics(t *testing.T) {
	st := testdb.New(t)
	m := metrics.New(st, testLog())

	// Empty history: enabled=1 (seed), interval=6h, last_success=0 (never).
