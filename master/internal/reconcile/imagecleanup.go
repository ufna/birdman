...
// РУЧНАЯ ревокация — store/models.go).
//
// Долгий офлайн вычищен ещё на входе: окружение с `down`-нодой ВООБЩЕ не приходит в
// выборку — его отсеивает store-фильтр VersionsPendingImageCleanup (batch-гигиена:
// заблокированное окружение не выедает limit-200-выборку у здоровых, иначе ≥200 его
// версий с самыми старыми created_at заморозили бы очистку всего парка). Поэтому
// «окружение с навсегда пропавшей машиной не чистит образы» теперь достигается на
// уровне выборки, а не пропуском тут (о пропаже кричит NodeDown-алерт; диск живых
// нод страхует watermark-GC). HasSession-гейт ниже остаётся боевым инвариантом для
// КОРОТКОГО офлайна (сессия упала, а node_down_after_min ещё не истёк — нода пока
// `active`/`quarantine`, версии в выборке) и как защита от гонки «выборка↔статус
// ноды».
// «Ни одной ноды в окружении» — вырожденный случай: удалять негде, отчётов ждать не
// от кого, маркер ставим сразу, иначе версия вечно крутилась бы в выборке.
