...
> Дата: 07.07.2026 (спеки поддерживаются в актуальном состоянии, пометки «уточнено в v0»). Спеки — источник истины для реализации итераций из `../05-runtime-iterations.md`. Расхождение кода со спекой = баг документации: чиним спеку в том же PR, что и код. Язык кода и идентификаторов — английский, доков — русский.
>
> **📍 Текущий статус реализации (что готово / что на боксе / доступ / дальше) — `../STATUS.md`.** Раздел «Что нужно для старта (итерация 0)» ниже — исторический (итерации 0–4 уже выполнены).
