...
| `GET/POST /v1/apikeys` · `DELETE /v1/apikeys/{id}` | admin | управление API-ключами (панель П2 Доступ; секрет — один раз) |
| `GET /v1/stats/overview` · `/v1/stats/cost` (`?days=N`) | readonly | агрегаты статистики/cost-view (панель П2; v0: on-the-fly) |
...
(Уточнено в v0, панель П2 — реализовано; тело — `internal/httpapi/{apikeys,stats,alerts}.go`, `store/{apikeys,stats}.go`; `server.go` тронут минимально: только маршруты + сеттер `WithAlertsSources`.)
