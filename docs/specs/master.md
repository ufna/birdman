...
| `GET /v1/environments/{project}/{name}/usage` | readonly | состав окружения `{versions, fleets, nodes, servers, matches, api_keys}` — что снесёт удаление (панель показывает его в диалоге). **Привязка ключа энфорсится** (#989, как на `/v1/matches/{id}` и `/v1/servers/{id}/logs`): привязанный ключ читает состав ТОЛЬКО своей пары, любая другая → `403 key is bound to <project>/<env>`. Гейт стоит ДО резолва (пара — сам адрес ручки, БД ему не нужна), поэтому 403 приходит БАЙТ-В-БАЙТ одинаково на существующее окружение, на выдуманное и на выдуманный проект: ЭТА РУЧКА оракулом существования не работает (имена окружений всё ещё выдают листинги — см. ниже и #993). `404` остаётся тому, кому пара разрешена: глобальному ключу и сессии панели, ЕСЛИ та заведена на глобальном ключе — сессия наследует привязку ключа целиком, а admin-ключ привязать нельзя (`CreateAPIKey`) |
...
| `GET /v1/metrics/query` · `/query_range` | readonly | read-only прокси к VictoriaMetrics для панели (уточнено в v0). **Только глобальный ключ** (#990): привязанному — 403, сырой PromQL нечем сузить по проекту (та же причина, что у logs-прокси ниже) |
| `GET /v1/logs/query` | readonly | read-only прокси к VictoriaLogs для панели: история/поиск логов по флоту (Логи v1). **Только глобальный ключ** (#990): привязанному — 403; иначе он читал бы через LogsQL ровно те байты чужого дедика, которые #988 закрыл на `/v1/servers/{id}/logs` |
...
**Границы: #989 закрыл РУЧКУ, а не enumerability окружений.** Привязка в API — поточечный гейт над адресуемым объектом, а НЕ арендаторская граница на чтениях. Листинги и агрегаты привязку не энфорсят вовсе: `projectFilter`/`scopeFilter` слаг только ВАЛИДИРУЮТ, и привязанный readonly-ключ читает через `?project=<чужой>` имена окружений (`/v1/environments`), hostname и `public_ip` нод, `image_ref` версий, матчи, ленту событий и `/v1/stats/*` (замерено); `/v1/alerts/active`·`/history` — то же, а `/v1/alerts/mutes` и `/rules` `?project=` не принимают вовсе и отдают всё безусловно. Более того, сама валидация `?env=` (#971) работает оракулом ИМЁН окружений: `?project=game&env=ghost` → `400 no such environment`, существующее → `200`. Поэтому «перебор имён чужих окружений» из карточки #989 закрыт ТОЛЬКО на этой ручке — как свойство API он остаётся, и одним запросом, без перебора. Сужать листинги привязкой — отдельное продуктовое решение, ломающее совместимость; замер и развилка — в карточке #993.
...
**Модель доступа обеих сырых проксий (tracker #990): passthrough открыт ТОЛЬКО глобальному ключу.** Ключ, привязанный к (project, env), получает `403 forbidden` `key is bound to <project>/<env>: raw query proxy is global-key only` и на `GET /v1/logs/query`, и на `GET /v1/metrics/query` · `/query_range` (`requireUnboundKey`, `httpapi/auth.go`). Почему дверь, а не сужение: у запроса нет объекта, чью пару можно сверить как в `requireBinding`, — есть произвольная программа на языке апстрима, и сузить её можно только ПЕРЕПИСАВ. Проектного ЛЕЙБЛА в стримах VictoriaLogs нет вовсе — в нашей разметке (`vector.yaml.j2`, `ops.md` §1) стрим несёт `server_id`/`node`/`region`; у self-host-оператора шиппер может быть свой. Это не значит «сузить нечем»: соответствие `server_id` → (project, env) master держит в БД и уже использует (`ServerLogTarget`, #988), а VictoriaLogs штатно умеет `extra_filters`/`extra_stream_filters` — но это сужение по множеству СВОИХ `server_id` (кардинальность живого флота) плюс вопрос истории, записанной без лейблов, то есть отдельная работа, а не строчка в хендлере. У VictoriaMetrics проектный лейбл несёт ЧАСТЬ серий (master-derived — `birdman_servers{project,env,…}`, `birdman_server_info{server_id,project,env}`), нодовые (vmagent ноды) — только `node`/`region`. Без этого гейта #988 закрывал одну дверь из двух: те же байты игрового вывода дедика чужого проекта доставались тому же readonly-ключу через `query={server_id="<uuid>"}` — отличие от закрытой ручки лишь в том, что uuid едет в query-строке, а не в пути. Фильтровать ОТВЕТ вместо запроса — защита от честного клиента, а не от атакующего: состав полей ответа задаёт сам запрос (пайпы проксируются как есть — панель уже шлёт `| sort by (_time) desc`), поэтому опираться на `server_id` в выдаче как на границу нельзя. Гейт стоит ДО проверки `victorialogs_url`/`victoriametrics_url` и ДО валидации `limit` (порядок #988: разрешение раньше состояния — 403 не должен рассказывать, подключена ли проксия). **Чего это НЕ делает:** проектной разметки стримов и серверного сужения запроса (правильная цель) здесь нет — отдельная карточка (tracker #994). Дёшево она не даётся: канал для переменных в `vector.yaml.j2` есть (`birdman_region` уже оттуда), но конфиг vector рендерит ansible, а истина живёт в БД master и меняется через API без перерендера (`PATCH /v1/nodes/{id} {env}`) — статический лейбл будет отставать от неё на новых строках; хост — вообще не тот гранулят, лейблить надо построчно (`server_id` → проект); строки, попавшие в VL ДО появления лейблов, останутся без них (вопрос миграции истории); а сужение самого запроса требует либо парсинга LogsQL/PromQL на master, либо `extra_filters`-подобной ручки апстрима — во втором случае энфорсмент уезжает туда, где тест master'а его не доказывает. **Периметр этого гейта — только СЫРОЙ passthrough.** Курируемые чтения, производные от тех же данных, привязку по-прежнему НЕ энфорсят: `GET /v1/alerts/active`/`history` отдают привязанному ключу алерты чужого проекта (не-скрывающий `?project=`, §6 выше), листинги `/v1/{servers,nodes,versions,events}` — чужие метаданные (tracker #995). Байтов игрового вывода среди них нет (он течёт ровно двумя путями — live-tail через агента и VL-проксия, оба закрыты), но считать периметр закрытым целиком нельзя. **Панель:** по `panel.md` §1 п.5 логин в неё — admin-ключом, для него не меняется ничего; но сессия наследует привязку (см. §6 выше), поэтому оператор, вошедший ПРИВЯЗАННЫМ ключом, увидит 403 на экране «Логи» и на графиках метрик — осознанная цена варианта «закрыть дверь сейчас» (на графиках это сейчас сырой английский текст мастера — tracker #996).
