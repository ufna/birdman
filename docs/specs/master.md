...
| `GET /v1/alerts/rules` · `/history` · `/active` (`?project=` — только `/history`·`/active`) | readonly | правила/история/активные алерты vmalert (панель П2). `project` (мультипроект W2, tracker #955) сужает **не скрывающе**: уходит только алерт ЯВНО чужого проекта, платформенный (без лейбла) виден всегда. `rules` — каталог конфигурации, не фильтруется |
...
Аутентификация: `Authorization: Bearer <api-key>`; скоупы из таблицы `api_keys`. Скоуп `matchmaking` ограничен тремя ручками тикетов, но внутри них почти не ограничен: любой `player_id`, чтение и отмена любого тикета по `ticket_id` (привязка ключа к (project, env) проверяется только на создании тикета, `httpapi/matchmaking.go`). Поэтому ключ рассчитан на **бэкенд игры**, а не на игровой клиент: вшить его в клиент = раздать общий секрет и потерять доверие к `player_id` (rate-limit — 5 rps per player_id, in-memory, это глушилка абьюза, не граница безопасности; per-IP лимита в master нет). Разбор границ — `architecture.md`, «Модель доверия (trust boundaries)».
...
(Уточнено в v0, панель П2 — реализовано; тело — `internal/httpapi/{apikeys,stats,alerts}.go`, `store/{apikeys,stats,alerts}.go`; `server.go` тронут минимально: только маршруты + сеттеры `WithAlertsSources`/`WithSilenceMirror`. Mute алертов — таблица `alert_mutes` (миграция `000004`; `silence_id` — `000016`), зеркалирование в alertmanager — `internal/amsilence`.)
