...
>
> **(Уточнено в v0, Реестры v1 — гейт бэкапов; расширено в v1, mTLS.)** Таблица `registries` (`master.md` §1) хранит токен реестра **plaintext** (принятый риск — master обязан отдавать его агентам по agentlink, `protocol.md` §1); ночной `pg_dump` уносит этот plaintext в дамп наравне с остальной БД (bcrypt-хэши ключей/нод там и так были, но это обратимый секрет, а не хэш). **Тот же гейт теперь покрывает и приватный ключ внутренней CA** (`internal_ca.key_pem`, `master.md` §1) — тоже обратимый секрет в дампе, тот же класс риска. **Гейт: оффсайт-выгрузку дампов (S3, выше) не включать, пока дампы не шифруются (age/gpg) либо оба секрета (registry-токены + ключ CA) не зашифрованы at-rest** — иначе первый же оффсайт-синк уносит их за периметр бокса. Пока дампы только локальные (см. выше), риск ограничен диском самого бокса.
- Потеря master-тачки: новая тачка → `master.yml` плейбук → restore дампа → агенты переподключаются сами; матчи, шедшие во время отказа, доигрываются (сервера живы), их `match_end` попадёт в PG после восстановления. Целевой RTO ≤ 60 мин, RPO ≤ 1 ч. (Уточнено в v1, mTLS.) Внутренняя CA живёт в PG (`internal_ca`), поэтому едет в том же дампе — восстановленный master поднимает **ту же** CA, и уже выданные клиентские серты нод остаются валидны: флот реконнектится mTLS **без ре-энроллмента**. Runbook §5 не меняется ни на строку (репетиция покрыта integration-тестом `master/internal/agentlink`).
