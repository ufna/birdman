...
>
> 1. **Инфра (`birdman_monitoring_dev`)**: vmalert `-notifier.url` → **всегда** alertmanager; alert-sink — **всегда** поднят и подключён как постоянный `webhook_configs`-receiver alertmanager'а (пишет `alerts.log` как раньше), `discord_configs` добавляется в тот же receiver при заданном webhook'е. Это заодно чинит латентную дыру текущего webhook-пути: сейчас при включённом Discord sink выпадает из цепочки и `alerts.log` (история панели) перестаёт пополняться. Alertmanager получает data-volume (персистентность silences между рестартами); `group_wait` подбирается так, чтобы acceptance «NodeDown в alerts.log ≤60с» держался. Sink нормализует alertmanager-webhook-тело (объект с полем `alerts`) к прежнему формату строки — парсер master не меняется и остаётся обратно-совместим со старыми строками лога.
