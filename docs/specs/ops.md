...
>
> **(Уточнено в v0, Реестры v1 — гейт бэкапов.)** Таблица `registries` (`master.md` §1) хранит токен реестра **plaintext** (принятый риск — master обязан отдавать его агентам по agentlink, `protocol.md` §1); ночной `pg_dump` уносит этот plaintext в дамп наравне с остальной БД (bcrypt-хэши ключей/нод там и так были, но это обратимый секрет, а не хэш). **Гейт: оффсайт-выгрузку дампов (S3, выше) не включать, пока дампы не шифруются (age/gpg) либо токены реестров не зашифрованы at-rest** — иначе первый же оффсайт-синк уносит plaintext registry-токены за периметр бокса. Пока дампы только локальные (см. выше), риск ограничен диском самого бокса.
