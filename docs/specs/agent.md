...
- (Уточнено, tracker #1065.) Эхо — **ресурс ХОСТА, а не ноды**. На боксе, несущем несколько нод birdman, порт держит **ровно один** агент, остальные ставят `qos_echo_addr: off` (§10). Причина не в экономии портов: респондер безадресный — он не несёт ни идентичности ноды, ни проекта, — а меряется им сетевой путь **до хоста**, одинаковый для всех его нод. Вдобавок `GET /v1/qos` сужен по (project, env) и берёт `select distinct region, host(public_ip)`, то есть отдаёт **один** таргет на пару (регион, ip): пер-нодовые порты выдали бы клиенту N неразличимых по rtt целей и сломали бы инвариант «единственный внешне-открытый порт ноды» (`ops.md` §4). Второй респондер на том же порту просто не забиндился бы — кто победит, решала бы гонка старта, а проигравший писал бы ошибку в лог вечно. **Цена, названная явно:** пока агент-владелец лежит, ping-таргет бокса тёмный для всех его нод; случай наблюдаем — на самой ноде-владельце горит `NodeDown`.
...
qos_echo_addr: ":19999"     # (§8) `off` — эхо на этом хосте держит ДРУГАЯ нода
