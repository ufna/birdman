...
**Lease/карантин:** master считает тачку живой при heartbeat моложе **10с**; тишина → `quarantine` (аллокации исключены, сервера → failed после ещё 20с); возвращение heartbeat → сверка карты серверов и `active`. (Уточнено в v0: «сверка карты» = только Hello-репорты могут воскресить `failed`-сервер обратно в `ready/allocated/draining` — дедики переживают тишину линка, master был пессимистичен; событие `server_recovered`. Обычные heartbeat'ы терминальные состояния не трогают; `reaped` не воскресает никогда.) **(Уточнено в follow-ups итерации 5 — третья ступень `down`.)** `quarantine` дольше `node_down_after_min` (деф. **10м**) → нода `down` + событие `node_down` (`MarkDownNodes`; оператор/алерты отличают моргнувшую ноду от лежащей давно). Вернувшийся heartbeat живой агент-сессии поднимает ноду из `quarantine`|`down` обратно в `active` (событие `node_recovered`). `dead` — ручная ревокация — автоматикой никогда не выставляется и heartbeat'ом не поднимается.
...
(Уточнено в v0, закрыто в итерации 6: `allocated.metadata` — полезная нагрузка ВНЕШНЕГО матчмейкера (`POST /v1/allocate {metadata}` → `AllocateServer.metadata` → этот фрейм, агент форвардит дословно и не читает; реплей несёт её же, потому что запоминается сам фрейм). Внутренний матчмейкер поле не заполняет; liba по-прежнему трактует отсутствие как пустой словарь. Капы на REST-входе: ≤16 ключей, ключ ≤64 байт, значение ≤512 байт.)
