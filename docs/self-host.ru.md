...
новейших N уходят в `disabled`, их образы снимаются с нод окружения сразу
(`RemoveImage`); watermark-GC диска остаётся страховкой. `active`/`prepulling`/
`deprecated` не трогаются никогда — окно отката остаётся тёплым.
