...
- **Логи без пары привязанному ключу не отдаются.** Пара едет в стрим-лейблы
  ...
  ретенции.
- **Разметку включает нода.** Агент кладёт лог в путь с парой только при
  `log_scope_dirs: true` в его конфиге (`agent.yaml`); ansible-роль
  `birdman_agent_dev` ставит флаг вместе с новым конфигом vector'а
  (`birdman_log_scope_dirs`, деф. `true`). Порядок не случаен: бинарь агента
  обновляется сам, конфиг шиппера — нет, и агент, начавший писать в
  подкаталоги раньше своего шиппера, перестал бы отправлять логи вообще.
  Пока флаг выключен, привязанный ключ получает `200` с пустой выдачей.
