...
серверов версии не осталось, и штампует `versions.image_cleanup_at` (повторов больше
не будет). Watermark-GC диска остаётся страховкой (он же покрывает узкий случай
рестарта мастера между отправкой и ack агента). `active`/`prepulling`/`deprecated`
не трогаются никогда — окно отката остаётся тёплым.
