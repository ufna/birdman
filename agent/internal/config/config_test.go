...
// qos_echo_addr: off — явный отказ ЭТОГО агента отвечать на QoS-пробы
// (tracker #1068: «эхо держит сосед» больше не настраивается — за это отвечает
// состязание за порт в qosecho). Отсутствие ключа обязано остаться :19999
// (иначе тихо погас бы ping-таргет всех существующих нод), а сентинел не должен
// зависеть от регистра и пробелов — его пишет шаблон роли, а читают люди.
