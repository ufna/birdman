...
/* eslint-disable react-refresh/only-export-components -- чистые маппинги «состояние → тон» (toneOfNodeState и соседи) и таблица EVENT_KINDS живут рядом с бейджем, который их применяет, и покрыты юнит-тестами напрямую. Правило про гранулярность Fast Refresh в dev-сервере, не про корректность; разносить файл по модулям ради него дороже, чем оно стоит. Политика — в eslint.config.js. */
