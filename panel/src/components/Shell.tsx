 ...
 * себе этот ответ неполон (см. `effectiveSectionOf` ниже). Это не делает
 * половинчатый вопрос незадаваемым — назвав admin-сессию, его получают обратно
 * (так и делает тест разбора пути), — но задать его теперь можно только
 * НАРОЧНО, а не по недосмотру.
 *
...
/**
 * Раздел, ДОСТУПНЫЙ этой сессии, — то есть тот, который роутер и отрендерит.
 ...
 *
 * Граница ответа, чтобы имя не читалось шире кода: это раздел ВНУТРИ
 * оболочки. Права панель гейтит ДВАЖДЫ, и второй гейт не здесь — `canRead` в
 * `Root` (App.tsx) отсекает сессию без `readonly`/`admin` до всякого роутера:
 * ключу со скоупом, скажем, только `deploy` (скоупы выдаются по SCOPE_OPTIONS
 * в Access.tsx) панель показывает карточку `app.noScope`, и ни Shell, ни нав,
 * ни скоуп не монтируются вовсе. Эта функция про такую сессию всё равно
 * ответит `/fleet` — и будет права ровно в том смысле, в каком её спрашивают:
 * «если бы оболочка рендерилась, то какой раздел».
 *
 ...
 * которым нав его вырезает) — значит его нет и в оболочке, отдаём Обзор.
 * Поэтому гейт РАЗДЕЛОВ не переписывается руками в роутере per-section: новый
 * admin-only пункт нава отгейтится сам. Самовыводится ровно эта ось и ничего
 * больше: классификацию скоупа новому разделу всё равно припишут руками
 * (множества ниже), а ссылки на admin-only разделы по-прежнему гейтятся своими
 * `canAdmin` по месту (например в DeployHowto).
 *
