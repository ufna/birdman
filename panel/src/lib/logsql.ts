...
export function escapeStreamValue(v: string): string {
  // replaceAll с обычной строкой, не regex-литералом: regex вида /"/g путает
  // наивный стриппер комментариев guard'а no-hardcoded.test.ts (он не знает
  // о regex-литералах и принимает символ " внутри них за открывающую кавычку
  // строки — дальше до следующей " в файле все строки считаются «внутри
  // строки» и не чистятся от кириллицы, что ложно триггерит guard).
